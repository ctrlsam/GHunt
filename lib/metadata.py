        ...
        try:
            ...
            for (idx, tag) in TAGS.items():
                ...
                location = self.geolocator.reverse(
                    "{}, {}".format(geoaxis["Latitude"],
                                    geoaxis["Longitude"])
                ).raw["address"]

        # TODO: What can trigger this - make it specific to error.
        except Exception:
            return
            ...
            for model, data in devices.items():
                ...
                '''for validity, dateslist in data["History"].items():
                    if dateslist and (
                        ...
                        print(f"{bkn if data['Firmwares'] else ''}- \
                               "{make.capitalize()} {model} ({n} pic{picx(n)}) [{dates}]")
                '''
                if data["Firmwares"]:
                    n = len(data['Firmwares'])
                    print(f"-> {n} Firmware{picx(n)} found !")
                    valid_check(data['Firmwares'])

        locations = self.locations
        if locations:
            print(f"\n[+] {len(locations)} location{picx(len(locations))} found !")
            valid_check(locations)

        softwares = self.softwares
        if softwares:
            print(f"\n[+] {len(softwares)} software{picx(len(softwares))} found !")
            valid_check(softwares)

        if not devices and not locations and not softwares:
            print("=> Nothing found")


def valid_check(items):
    ''' Outputs if the data provided is valid or not '''
    for var, data in items.items():
        
        n = len(data["Valid"] + data["Invalid"])
        for validity, dateslist in data.items():
            if not dateslist and not((validity == "Valid") or (validity == "Invalid" and not data["Valid"])):
                continue
            ...
            if validity == "Valid":
                ...
                dates = "?"
            print(f"- {var} ({n} pic{picx(n)}) [{dates}]")